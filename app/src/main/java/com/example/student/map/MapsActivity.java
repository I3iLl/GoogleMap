...
public class MapsActivity extends FragmentActivity implements GoogleMap.OnMapLongClickListener, LocationListener {
    ...
    int place = 0;
    LatLng PLACE, lastPLACE=null;

    private void setUpMap() {

        /*
        LatLng BKK = new LatLng(13.736717,100.523186);
        mMap.addMarker(new MarkerOptions().position(BKK).title("Place " + ++place));
        lastPLACE = BKK;
        ...
        */
        ...
        /*
        LatLng BKK1 = new LatLng(13.736717,100.523186);
        LatLng BKK2 = new LatLng(15.736717,102.523186);

        mMap.addMarker(new MarkerOptions().position(BKK1).title("Bangkok"));
        mMap.addMarker(new MarkerOptions().position(BKK2).title("Hongkong"));

        CameraUpdate start_map = CameraUpdateFactory.newLatLng(BKK1);
        mMap.moveCamera(start_map);

        CameraUpdate start_zoom = CameraUpdateFactory.zoomTo(7);
        mMap.moveCamera(start_zoom);

        mMap.addPolyline(new PolylineOptions().add(BKK1).add(BKK2));
        */
    ...
    public void onMapLongClick(LatLng latLng) {
        ...
        if(lastPLACE != null) {
            mMap.addPolyline(new PolylineOptions().add(lastPLACE).add(latLng));
        }
    ...
    public void onLocationChanged(Location location) {
        ...
        CameraUpdate move = CameraUpdateFactory.newLatLng(PLACE);
